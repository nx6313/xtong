! function (a) {
  ...
  a.fn.circleChart = function (b) {
    ...
    d.prototype = {
      ...
      drawBackground(a) {
        a.beginPath(), a.arc(this.pos * c.tScale, this.pos * c.tScale, this.settings.backgroundFix ? .9999 * this.radius * c.tScale : this.radius * c.tScale, 0, 2 * Math.PI), a.lineWidth = this.settings.backgroundFix ? .4 * this.lineWidth : this.lineWidth, a.strokeStyle = this.settings.backgroundColor, a.stroke()
      },
      draw(a) {
        if (a.beginPath(), this.settings.counterclockwise) {
          let b = 2 * Math.PI;
          a.arc(this.pos * c.tScale, this.pos * c.tScale, this.radius * c.tScale, b - this.bAngle, b - (this.bAngle + this.cAngle), this.settings.counterclockwise)
        } else a.arc(this.pos * c.tScale, this.pos * c.tScale, this.radius * c.tScale, this.bAngle, this.bAngle + this.cAngle, this.settings.counterclockwise);
      ...
      animate(a, b, c, d, e) {
        ...
        f < 1 && (f = 1), c - d < 1.05 * this.settings.speed && (!e && 1e3 * this.cAngle <= Math.floor(1e3 * this.eAngle) || e && 1e3 * this.cAngle >= Math.floor(1e3 * this.eAngle)) ? (this.cAngle = Math[this.settings.animation]((c - d) / f, this.sAngle, this.eAngle - this.sAngle, this.settings.speed / f), b.clearRect(0, 0, this.settings.size * this.settings.tScale, this.settings.size * this.settings.tScale), this.settings.background && this.drawBackground(b), this.draw(b), this.onDraw(a), c = (new Date).getTime(), j(() => this.animate(a, b, c, d, e))) : (this.cAngle = this.eAngle, b.clearRect(0, 0, this.settings.size * this.settings.tScale, this.settings.size * this.settings.tScale), this.settings.background && this.drawBackground(b), this.draw(b), this.setCurrentAnglesData(a))
      },
      setCurrentAnglesData(a) {
        var b = {
          percent: i,
    ...
    return this.each((e, f) => {
      ...
      a("canvas.circleChart_canvas", i).length || i.append(function () {
        return a("<canvas/>", {
          ...
          width: m.size * c.tScale,
          height: m.size * c.tScale
        }).css(m.autoCss ? {
          "margin-left": "auto",
          "margin-right": "auto",
          width: m.size + "px",
      ...
      m.animate ? 0 !== m.value ? x.animate(i, o, (new Date).getTime(), (new Date).getTime(), t > s) : j(() => {
        o.clearRect(0, 0, this.settings.size * this.settings.tScale, this.settings.size * this.settings.tScale);
        x.settings.background && x.drawBackground(o)
      }) : (x.cAngle = x.eAngle, j(() => {
        m.background && x.drawBackground(o);
        0 !== m.value ? (x.draw(o), x.onDraw(i), x.setCurrentAnglesData(i)) : (o.clearRect(0, 0, this.settings.size * this.settings.tScale, this.settings.size * this.settings.tScale), x.settings.background && x.drawBackground(o))
      }))
    })
  }
}(jQuery);
